...
// The phone's line to the notesfile (RDVAX::GRATEFUL on the web), which is
// where the Apple ID's shelves live. One bearer token in the Keychain;
// everything else is JSON over HTTPS.
...
nonisolated struct AppleSignInResponse: Codable, Sendable { var token: String; var head: NetheadHead }
...
nonisolated struct NetheadAPIClient: Sendable {
    ...
    /// A nonce for one Apple sign-in; the request carries its SHA-256.
    func appleNonce() async throws -> String {
        struct Reply: Decodable { var nonce: String }
        let reply: Reply = try await send("GET", "api/apple/nonce", body: Optional<String>.none, auth: false)
        return reply.nonce
    }

    private nonisolated struct AppleBody: Encodable, Sendable { var identityToken: String; var nonce: String; var name: String?; var device: String }

    /// Apple's identity token → the notesfile's session token.
    func signInWithApple(identityToken: String, nonce: String, name: String?, device: String) async throws -> AppleSignInResponse {
        let reply: AppleSignInResponse = try await send("POST", "api/apple", body: AppleBody(identityToken: identityToken, nonce: nonce, name: name, device: device), auth: false)
