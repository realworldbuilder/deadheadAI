...
// tonight's show in Nethead" works from the road.

struct PlayTonightsShowIntent: AudioPlaybackIntent {
    static let title: LocalizedStringResource = "Play Tonight's Show"
    static let description = IntentDescription("Plays Nethead's featured Grateful Dead show of the day.")

    @MainActor
    func perform() async throws -> some IntentResult & ProvidesDialog {
        guard let env = AppEnvironment.current else {
            return .result(dialog: "Nethead isn't ready yet — open the app once and try again.")
        }
        ...
        guard let hero = env.knowledgeBase.heroShow(dayOfYear: dayOfYear,
            ...
            return .result(dialog: "Couldn't pick tonight's show. Open Nethead and try from Home.")
        }
        let detail = try await env.metadataProvider.detail(for: recording.identifier)
        guard !detail.tracks.isEmpty else {
            return .result(dialog: "That tape wouldn't stream. Open Nethead to pick another.")
        }
...
struct PlayShowOnDateIntent: AudioPlaybackIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        guard !detail.tracks.isEmpty else {
            return .result(dialog: "The tape from \(day) wouldn't stream. Open Nethead to pick another source.")
        }
...
struct NetheadShortcuts: AppShortcutsProvider {
