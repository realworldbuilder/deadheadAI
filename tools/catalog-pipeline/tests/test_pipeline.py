...
def _tiny_catalog(path):
    import sqlite3
    from stage4_build import SCHEMA
    db = sqlite3.connect(path)
    db.executescript(SCHEMA)
    db.execute("INSERT INTO catalog_meta VALUES('schema_version','3')")
    db.execute("INSERT INTO shows VALUES('1977-05-08','1977-05-08',1977,5,8,'hiatus-return',"
               "'Barton Hall','Ithaca','NY','full',2,'gd77-05-08.sbd','SBD',4.8,312,900000,NULL)")
    db.execute("INSERT INTO shows VALUES('1972-05-04','1972-05-04',1972,5,4,'europe-wall',"
               "'Olympia Theatre','Paris','France','full',1,'gd72-05-04.sbd','SBD',4.7,40,20000,NULL)")
    db.execute("INSERT INTO recordings VALUES('gd77-05-08.sbd','1977-05-08','Cornell','SBD',"
               "'SBD> reel'||char(10)||'line two',NULL,'Miller',4.8,312,900000,4100.0)")
    db.execute("INSERT INTO recordings VALUES('gd77-05-08.aud','1977-05-08','Cornell aud','AUD',"
               "NULL,NULL,NULL,4.1,3,1000,2000.0)")
    db.execute("INSERT INTO recordings VALUES('gd72-05-04.sbd','1972-05-04',NULL,'SBD',"
               "NULL,NULL,NULL,4.7,40,20000,4090.0)")
    db.execute("INSERT INTO recording_tracks VALUES('gd77-05-08.sbd',?)",
               ('[["Scarlet Begonias",581.0,"gd77-05-08d2t01.mp3"],["Fire on the Mountain",852.0,"gd77-05-08d2t02.mp3"]]',))
    db.execute("INSERT INTO setlist_entries VALUES('1977-05-08',0,'Set 2','scarlet begonias','Scarlet Begonias',1)")
    db.execute("INSERT INTO setlist_entries VALUES('1977-05-08',1,'Set 2','fire on the mountain','Fire on the Mountain',0)")
    db.execute("INSERT INTO songs VALUES('scarlet begonias','Scarlet Begonias',1,'1977-05-08','1977-05-08')")
    db.execute("INSERT INTO songs VALUES(\"truckin'\",\"Truckin'\",1,'1972-05-04','1972-05-04')")
    db.execute("INSERT INTO show_fts(rowid, blob) VALUES(1,'placeholder')")
    db.commit()
    db.close()


def test_stage7_roundtrip(tmp_path):
    import sqlite3
    import stage7_d1
    src = tmp_path / "cat.sqlite"
    _tiny_catalog(src)
    out = tmp_path / "d1"
    manifest = stage7_d1.dump(src, out, single=True)
    sql = (out / "catalog.tables.sql").read_text() + (out / "catalog.fts.sql").read_text()
    for line in sql.splitlines():
        assert line.endswith(";"), line[:80]
        assert not line.startswith(("BEGIN", "COMMIT", "PRAGMA"))
    db = sqlite3.connect(":memory:")
    for line in sql.splitlines():
        db.execute(line)
    assert db.execute("SELECT COUNT(*) FROM shows").fetchone()[0] == 2
    assert db.execute("SELECT COUNT(*) FROM recordings").fetchone()[0] == 3
    assert manifest["row_counts"]["recordings"] == 3
    # newlines survive the char(10) rendering
    assert db.execute("SELECT source_text FROM recordings WHERE identifier='gd77-05-08.sbd'").fetchone()[0] == "SBD> reel\nline two"
    # the contentless FTS was rebuilt and understands head-style dates
    hit = db.execute("SELECT rowid FROM show_fts WHERE show_fts MATCH '\"5-8-77\"*'").fetchone()
    assert hit is not None
    show = db.execute("SELECT show_id FROM shows WHERE rowid=?", (hit[0],)).fetchone()[0]
    assert show == "1977-05-08"
    # derived tables
    assert db.execute("SELECT show_count FROM web_year_counts WHERE year=1977").fetchone()[0] == 1
    assert db.execute("SELECT song_key FROM web_song_slugs WHERE slug='truckin'").fetchone()[0] == "truckin'"
    assert db.execute("SELECT COUNT(*) FROM web_search_blob").fetchone()[0] == 2
    assert db.execute("SELECT value FROM web_meta WHERE key='schema_version'").fetchone()[0] == "3"


def test_stage7_chunks_stay_under_limit(tmp_path):
    import sqlite3
    import stage7_d1
    src = tmp_path / "big.sqlite"
    _tiny_catalog(src)
    db = sqlite3.connect(src)
    huge = '[' + ','.join('["%s",100.0,"f%d.mp3"]' % ("x" * 40, i) for i in range(900)) + ']'
    assert len(huge) > 50_000
    db.execute("INSERT INTO recording_tracks VALUES('gd72-05-04.sbd',?)", (huge,))
    db.commit()
    db.close()
    out = tmp_path / "d1"
    stage7_d1.dump(src, out, single=True)
    for line in (out / "catalog.tables.sql").read_text().splitlines():
        assert len(line.encode("utf-8")) < 100_000
