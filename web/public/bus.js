...
/* Sign in with Apple on the web: Apple's popup hands back an identity token,
   we hand it to /auth/apple with the nonce we were given. */
(function () {
  var D = document, client = D.body.getAttribute('data-apple-client');
  var buttons = D.querySelectorAll('[data-apple="signin"]');
  if (!buttons.length) return;
  if (!client || !window.AppleID) { buttons.forEach(function (b) { b.closest('.btns').hidden = true; }); return; }
  function post(url, body) { return fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }).then(function (r) { return r.json().then(function (j) { if (!r.ok) throw new Error(j.error || 'Bummer.'); return j; }); }); }
  function fail(msg) { var e = D.querySelector('[data-error]'); if (e) { e.hidden = false; e.textContent = msg; } }
  buttons.forEach(function (b) {
    b.addEventListener('click', function () {
      b.disabled = true;
      fetch('/auth/apple/nonce').then(function (r) { return r.json(); }).then(function (n) {
        AppleID.auth.init({ clientId: client, scope: 'name', redirectURI: location.origin + '/auth/apple', nonce: n.nonce, usePopup: true });
        return AppleID.auth.signIn().then(function (res) {
          var name = res && res.user && res.user.name ? res.user.name.firstName : undefined;
          return post('/auth/apple', { idToken: res.authorization.id_token, nonce: n.nonce, name: name, next: b.getAttribute('data-next') });
        });
      }).then(function (j) { location.href = j.next || '/me'; })
        .catch(function (e) { b.disabled = false; if (e && (e.error === 'popup_closed_by_user' || e.error === 'user_cancelled_authorize')) return; fail((e && e.message) || "Apple didn't come through. Try again."); });
    });
  });
})();
