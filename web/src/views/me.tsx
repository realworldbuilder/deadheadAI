...
export const Me: FC<{ head: Head; saved?: boolean; welcome?: boolean; error?: string | null; passkeyCount: number; counts: { notes: number; shelves: number; mixtapes: number; journal: number }; providers: string[]; canRename?: boolean }> = ({ head, saved, welcome, error, passkeyCount, counts, providers, canRename }) => (
  <>
    <h1 class="topic">{head.handle}</h1>
    <p class="sub">Your page. <a href={`/heads/${head.handle}`}>How heads see it</a>.{providers.includes("apple") ? " Signed in with Apple." : ""}</p>
    {welcome ? <Ok>You're on. Your shelves follow you: sign in with the same Apple ID in Nethead on your iPhone and they're there.</Ok> : null}
    {saved ? <Ok>Saved.</Ok> : null}
    {error ? <Err>{error}</Err> : null}
    {canRename ? (
      <form method="post" action="/me/handle">
        <label for="handle">Your handle</label>
        <input id="handle" name="handle" value={head.handle} placeholder="NODE::NAME" autocapitalize="characters" aria-describedby="handle-help" />
        <p class="help" id="handle-help">We picked one for you. Change it once, before you write a note; after that it stands.</p>
        <div class="btns"><button class="btn" type="submit">Keep this handle</button></div>
      </form>
    ) : null}
    ...
    <h2>The Phone</h2>
    <p>Nethead on your iPhone: sign in with the same Apple ID under Settings, and your shelves are the same there and here.</p>
