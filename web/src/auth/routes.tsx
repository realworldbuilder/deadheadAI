...
auth.get("/bus", (c) => {
  ...
  return c.html(render(<Frame title="Get on the Bus" crumb={["Get on the Bus"]} head={null} page="bus" bus><Bus /></Frame>));
});

auth.post("/bus", async (c) => {
  const form = await c.req.parseBody();
  return c.html(render(<Frame title="Get on the Bus" crumb={["Get on the Bus"]} head={null} page="bus" bus>
...
auth.get("/signin", (c) => {
  ...
  return c.html(render(<Frame title="Sign in" crumb={["Sign in"]} head={null} page="signin" bus><SignIn next={safeNext(c.req.query("next"))} /></Frame>));
});

auth.post("/signin", async (c) => {
  return c.html(render(<Frame title="Sign in" crumb={["Sign in"]} head={null} page="signin" bus><SignIn error={NO_JS} /></Frame>), 400);
});
...
auth.get("/recover", (c) => c.html(render(<Frame title="Back on the bus" crumb={["Recover"]} head={null} page="signin" bus><Recover /></Frame>)));
...
auth.post("/recover", async (c) => {
  ...
  const page = (error: string, status: 400 | 429 = 400) =>
    c.html(render(<Frame title="Back on the bus" crumb={["Recover"]} head={null} page="signin" bus><Recover handle={rawHandle} error={error} /></Frame>), status);
  ...
  return c.html(render(<Frame title="Back on the bus" crumb={["Recover"]} head={null} page="signin" bus>
